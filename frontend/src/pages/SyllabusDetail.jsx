...
export default function SyllabusDetail() {
  ...
  useEffect(() => {
    api.get(`/api/v1/public/syllabus/${slug}`)
      .then(res => {
        setItem(res.data);
        setLoading(false);
      })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow max-w-sm">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Syllabus Not Found</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-screen">
      <Link to="/" className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-teal-600 mb-6 bg-white border px-3 py-2 rounded-xl transition">
        <ArrowLeft size={12} />
        Back to Home
      </Link>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100/60 space-y-6">
        ...
        {(item.downloadLink || item.officialLink) && (
          <div className="mt-12 border border-teal-100 bg-teal-50/20 rounded-2xl p-6">
            <h3 className="text-md font-extrabold text-teal-950 mb-4 flex items-center gap-2">
              <ExternalLink size={18} className="text-teal-600" /> Useful Important Links Area
            </h3>
                ...
                <tbody>
                  {item.downloadLink && (
                    <tr className="border-b border-gray-100">
                      <td className="p-4 text-gray-700 bg-gray-50/50 w-1/2">Download Syllabus PDF file</td>
                      <td className="p-4">
                        <a 
                          href={item.downloadLink} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="bg-teal-600 text-white font-bold text-xs px-4 py-2 rounded-lg hover:bg-teal-700 transition inline-block uppercase tracking-wider"
                        >
                          Click Here
                        </a>
                      </td>

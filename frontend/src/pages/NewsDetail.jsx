...
export default function NewsDetail() {
  ...
  useEffect(() => {
    api.get(`/api/v1/public/news/${slug}`)
      .then(res => {
        setItem(res.data);
        setLoading(false);
      })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow max-w-sm">
          <h2 className="text-xl font-bold text-gray-800 mb-4">News Article Not Found</h2>
          <Link to="/news" className="bg-blue-600 text-white px-6 py-2 rounded-xl">Back to News</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-screen">
      <Link to="/news" className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-rose-600 mb-6 bg-white border px-3 py-2 rounded-xl transition">
        <ArrowLeft size={12} />
        Back to News List
      </Link>

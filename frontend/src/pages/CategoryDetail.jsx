...
export default function CategoryDetail() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get(`/api/v1/public/categories/${slug}`)
      .then(res => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const { category, schemes } = data;

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-screen">
      <div className="mb-12">
        <span className="text-blue-600 text-xs font-bold uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full">Category</span>
        <h1 className="text-4xl font-extrabold text-gray-900 mt-2 tracking-tight">
          {category.name} {category.hindiName ? `(${category.hindiName})` : ''}
        </h1>
        <p className="text-gray-500 text-sm mt-1">Explore all central and state government programs listed under {category.name}.</p>
      </div>

      {schemes.length === 0 ? (
        <div className="bg-white p-12 rounded-2xl border border-gray-100 shadow-sm text-center">
          <p className="text-gray-500">No schemes are currently active under this category.</p>
        </div>
      ) : (

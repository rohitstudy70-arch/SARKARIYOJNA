...
export default function ResultDetail() {
  ...
  useEffect(() => {
    api.get(`/api/v1/public/results/${slug}`)
      .then(res => {
        setItem(res.data);
        setLoading(false);
      })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow max-w-sm">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Result Announcement Not Found</h2>
          <Link to="/results" className="bg-blue-600 text-white px-6 py-2 rounded-xl">Back to Results</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-screen">
      <Link to="/results" className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-emerald-600 mb-6 bg-white border px-3 py-2 rounded-xl transition">
        <ArrowLeft size={12} />
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100/60 space-y-6">
        ...
        {(item.resultLink || item.officialLink) && (
          <div className="mt-12 border border-emerald-100 bg-emerald-50/20 rounded-2xl p-6">
            <h3 className="text-md font-extrabold text-emerald-950 mb-4 flex items-center gap-2">
              <ExternalLink size={18} className="text-emerald-600" /> Useful Important Links Area
            </h3>
                ...
                <tbody>
                  {item.resultLink && (
                    <tr className="border-b border-gray-100">
                      <td className="p-4 text-gray-700 bg-gray-50/50 w-1/2">Download Scorecard / Result Sheet</td>
                      <td className="p-4">
                        <a 

...
export default function ListPage() {
  ...
  const handleBulkAction = async (action: string) => {
    ...
    for (const id of selected) {
      if (action === 'delete') {
        await fetch(`/api/v1/admin/results?id=${id}`, { method: 'DELETE' });
      } else {
        await fetch(`/api/v1/admin/results`, {
          method: 'PUT',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold capitalize">{ 'results'.replace('-', ' ') }</h2>
        <Link href="/portal/myadmin/results/create" className="bg-primary text-white px-4 py-2 rounded font-medium">+ Add New</Link>
      </div>
              ...
              {paginatedItems.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4 flex gap-3">
                    <Link href={`/admin/results/${item.id}`} className="text-blue-600 hover:underline">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>

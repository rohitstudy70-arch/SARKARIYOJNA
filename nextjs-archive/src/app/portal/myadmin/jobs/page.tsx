...
export default function ListPage() {
  ...
  const handleBulkAction = async (action: string) => {
    ...
    for (const id of selected) {
      if (action === 'delete') {
        await fetch(`/api/v1/admin/jobs?id=${id}`, { method: 'DELETE' });
      } else {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold capitalize">{ 'jobs'.replace('-', ' ') }</h2>
        <Link href="/portal/myadmin/jobs/create" className="bg-primary text-white px-4 py-2 rounded font-medium">+ Add New</Link>
      </div>
              ...
              {paginatedItems.map((item: any) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4 flex gap-3">
                    <Link href={`/admin/jobs/${item.id}`} className="text-blue-600 hover:underline">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
